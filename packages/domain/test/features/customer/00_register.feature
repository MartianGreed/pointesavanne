...
Feature: Customer registration
  ...
  Scenario: Registering with an email that already exists
    ...
    When the customer wants to register
    Then registration should fail with "IdentityConflict" and message "An authentication identity with that value already exists"
  ...
  Scenario: Registering with a bad password
    ...
    When the customer wants to register
    Then registration should fail with "AuthValidationError" and message "password: must contain at least 8 characters"