...
Feature: Estimation
  ...
  Scenario: Invalid booking dates
    ...
    Then an exception "ValidationFailed" should be thrown with message "End date 13/05/2022 cannot be before start date 30/05/2022"
  ...
  Scenario: Booking is not available for given time period
    ...
    When the customer submits the QuotationRequest
    Then an exception "ValidationFailed" should be thrown with message "Booking is unavailable for dates 30/05/2022 - 13/06/2022"
  ...
  Scenario: Simple booking for period
    ...
    And a "BookingRequested" event should have been dispatched
