...
export interface BookingRow {
  ...
  readonly status: "quotation-requested" | "quotation-awaiting-acceptation" | "quotation-signed" | "contract-sent"
...
export const frenchDate = (isoDay: string): string => {
...
export class BookingService {
  ...
  checkAvailability(villaId: string, from: string, to: string): Promise<{ available: boolean }> {
    return this.#api.get(`/bookings/availability?villaId=${encodeURIComponent(villaId)}&from=${from}&to=${to}`)
  }

  async uploadSignedQuotation(bookingId: string, file: File): Promise<{ bookingId: string; status: string }> {
    const contentBase64 = await this.fileToBase64(file)
    return this.#api.post(`/bookings/${bookingId}/signed-document`, { fileName: file.name, contentBase64 })
  }

  validateQuotation(bookingId: string, accepted: boolean, reason?: string): Promise<{ bookingId: string; status: string }> {
